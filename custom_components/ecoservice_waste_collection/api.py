...
class EcoserviceApi:
    ...
    def _discover_fields(schema: dict[str, Any]) -> tuple[str, str, str, str, str, str | None]:
        ...
        def walk(node: Any, entity: str = "") -> None:
            if isinstance(node, dict):
                ...
                for value in node.values(): walk(value, current)
            elif isinstance(node, list):
                for value in node: walk(value, entity)
        walk(raw)
        aliases = {
            "municipality": ("sav", "savivaldyb"), "address": ("adres",),
            "inventory": ("invent",), "collection": ("aptarn", "data", "date"),
    ...
    async def schedules(self, municipality: str, address: str, inventories: list[str]) -> dict[str, Schedule]:
        ...
        result = {i: Schedule(Container(i, waste_type_from_inventory(i)), normalize_dates(values)) for i, values in grouped.items()}
